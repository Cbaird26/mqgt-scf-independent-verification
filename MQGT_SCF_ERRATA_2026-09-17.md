...
**Date:** 2026-09-17 (v4 — author's public code located and run; E9 resolved
at source, E7 substantially revised)
...
v4 additionally uses the author's public code repository
`github.com/startigerjln/CenterforTopologicalPhysics`
(`tuft_spectrum.py`, retrieved and run 2026-09-17; output quoted below).
...
not a one-parameter fit to the normal-ordering floor. (v4: confirmed again
at code level — the author's `tuft_spectrum.py` neutrino section is
identical to printed Eq. (136) and reproduces our [B] output exactly.)
...
## E7. Charged-lepton masses (v3: one missing constant; v4: source code found — three mutually inconsistent versions, and the lepton scale is fit to m_e in the public code)
...
**E7 v4 — the author's public code changes the picture.**
`github.com/startigerjln/CenterforTopologicalPhysics/tuft_spectrum.py`
(retrieved and run 2026-09-17) implements a *different* lepton sector than
v5 prints:

1. **The lepton scale is fit to the electron mass.** The code sets
   Λ₃ = m_e^PDG / (2·exp(a − ζ(3) + β + α/6)) — its own comment reads
   "Λ set by m_e (spectral normalization)" and its summary line states
   "Leptons: Λ from spectral normalization (1 param)." The v5 text claims
   "the sole empirical input is the electroweak VEV v" for Theorem 34.
   In the public code that is not the case for the lepton sector: m_e is
   an input, so the lepton sector there is a 1-fit-parameter construction
   and only m_μ, m_τ are genuine outputs. The printed D(n) version (which
   with one unprinted constant hits all three leptons — v3 finding above)
   shows the zero-input version exists on paper, but it is not what the
   public code runs.
2. **The code's current numbers miss the muon.** Running the script
   verbatim gives m_μ = 105.660972 MeV vs PDG 105.6583755 ± 0.00024 —
   a **10.8σ** deviation (m_τ: 1776.864 vs 1776.86, 0.04σ ✓). The v5
   table prints m_μ = 105.658 at "0.00σ". So the v5 printed table was
   *not* generated by the public code.
3. **Three versions of the lepton sector now exist**, mutually
   inconsistent at the ~3×10⁻⁵ level: (i) v5 print — exact D(n) =
   (1.203011392, 4.806545406, 10.818228646), τ₃(trefoil) = √3, Λ_Hopf =
   √(2π)vκ⁶ with one missing constant; (ii) the public code — asymptotic
   ζ(3)n² plus a β·n(n+1)/2 knot term, τ₃(trefoil) = 3^{1/(3−σ₃√2)} =
   1.44994 (≠ √3 = 1.73205), Λ₃ fit to m_e; (iii) whatever version
   generated the v5 mass table. (Also minor: code uses v = 246219.65 MeV
   vs printed 246220.)
4. **What this means for the claim.** The n-dependent structure verified
   in v3 stands, and the v5 exact-D(n) formulation is the strongest
   version — but it needs its missing constant printed and its relation to
   the public code reconciled. A referee diffing the paper against the
   repo will find exactly this. Recommended repair: publish the
   D(n)-generating code (or the closed form for the exact D(n)), state
...
**E9 v4 — RESOLVED at source.** The author's public code computes
ΛB = v·√(2/r)·sin(π/r)·exp(−(2 + 1/(4π))·α): the suppression is
h∨ + 1/(4π) = 2 + 1/(4π), not the printed h∨ = 2. The extra factor
e^{−α/(4π)} = e^{−5.806×10⁻⁴} is exactly the missing assembly constant
class we measured (5.504×10⁻⁴ from the v5 printed table; the code's
factor overshoots the printed table by 3.0×10⁻⁵, i.e. the code and the
printed table differ slightly, but both sit well within PDG errors).
Running the author's script verbatim: W = 80366.97 MeV (0.17σ),
Z = 91184.97 (1.25σ), H = 125221.2 (0.05σ). Repair for the next edition:
print the full exponent −(2 + 1/(4π))α in Eq. (87) and derive the 1/(4π)
term (the text currently motivates only h∨ = 2 from "two charged
generators"; where 1/(4π) comes from is not stated in v5). With that,
the boson sector is complete at the same standard as the quark sector.
...
(2.16 MeV to 172.9 GeV) from one closed-form system. (v4: the author's
public code confirms every convention independently — including
λT(1) = 2/(3√3) and the n = 1 prefactor 2/3 — and its quark output
matches our [F] to ~10⁻⁶ relative.)
